...
describe('Broken Sauce', function () {
    ...
    it('should navigate to Sauce Labs homepage and click Documentation', async function () {
        ...
        try {
                ...
                console.log("Waiting for the exact Sauce Labs homepage link..."); 
                let sauceLabsLink = await driver.wait(
                until.elementLocated(By.css('a[href="https://saucelabs.com/"]')),50000);
                await sauceLabsLink.click();


                // Step 3: Wait for the "Resources" dropdown to be present
                console.log("Waiting for Resources dropdown to be present...");
                let resourcesDropdown = await driver.wait(
                    until.elementLocated(By.xpath("//button[contains(text(), 'Resources')]")),
                    10000
                );

                // Step 4: Hover over the "Resources" dropdown
                console.log("Hovering over Resources dropdown...");
                await driver.actions().move({ origin: resourcesDropdown }).perform();
